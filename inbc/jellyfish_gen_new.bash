...
do
  ...
  then
    ...
    while [ $sgn != "0" ]
    do
      timeout 300 ./jellyfish-linux count -m $2 -s $size -t 32 -o $filename.jf $file
      sgn=$?
      if [ $sgn != "0" ]
      then
        echo "Stuck. Retrying"
    ...
    while [ $sgn != "0" ]
    do
      timeout 300 ./jellyfish-linux count -m $2 -s $size -t 32 -C -o $filename.jf $file
      sgn=$?
      if [ $sgn != "0" ]
      then
        echo "Stuck. Retrying"
  ...
  ./jellyfish-linux dump -c $filename.jf > $filename.kmr
  rm $filename.jf
